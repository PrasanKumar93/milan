...
export function LineGrid({
  ...
  return (
    <>
        ...
        <table className="grid grid--zebra">
          ...
          <tbody ref={body}>
            {computed.lines.map((l, i) => {
              ...
              const last = i === computed.lines.length - 1;

              return (
                <tr
                  key={line.id}
                  /* Enter finishes the row and starts the next, a row with no
                   size in it yet being the end of the list rather than one
                   more to add. */
                  onKeyDown={
                    last ? (e) => onKeyDown(e, line.actualH === 0 && line.actualW === 0) : undefined
                  }
                >
      ...
      {/* The button sits at the right, under the end of the table it fills, and
          does exactly what Enter on the last row does — including leaving the
          cursor in the row it just made. */}
