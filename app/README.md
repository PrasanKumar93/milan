...
Browser app for preparing proforma invoices and downloading them as a PDF and a working Excel sheet. No server and no database: the operator fills a form, clicks download, and the files are the record.
...
`retype` needs `npm run dev` in another terminal. It opens the app in Chrome, types a sample's sizes, rates and charges as an operator would, and compares every cell the app worked out for itself against the figures on the original PDF — chargeable sizes, areas, amounts, charges, totals, GST and the grand total — then screenshots both tabs and presses download, saving the PDF and the workbook it is handed into `/tmp/retype`. The tests prove the arithmetic; this proves the screen is wired to it. `npm run retype -- all` sweeps every sample in one browser and prints only what differs; the quotes that do not come out identical are the documented ones in dev-plan §9 — hand-typed areas, ±₹1 roundings and two mirror lines charged over the rule.
...
Because nothing is filed away, the workbook is the reopen button — which is why **Download PDF + Excel** is one button and not two. The two files are not alternatives: the PDF is what the customer is sent, and the workbook is the only way the quote can be changed afterwards, so the office takes both of every quote and the choice was one nobody was making. One press writes both under the one name — `PROFORMA-7178.pdf` and `PROFORMA-7178.xlsx` — and then clears the draft. A browser asks once whether the site may save more than one file at a time; after Allow, it never asks again.

The sheet is the page the PDF prints — same columns, same boxed figures, the mark, the stamp, the total on yellow — but every derived figure is a live formula: change a size, a rate or a quantity and the chargeable size, area, amount, subtotal, GST and totals all follow, using the same rules as the engine. The wastage allowance and the unit rate behind a counted charge are in hidden columns so the formulas have their inputs and the printed page stays the document the customer knows; it is set to A4 and fit to one page wide, so printing from Excel gives the proforma.
