...
**The printed head names the unit too: `ACTUAL SIZE (MM)` and `CHARGEABLE (MM)`, or `(INCH)` on an inch quote.** The office's own sheet leaves it off, and on a page of bare numbers the unit is guessed — as millimetres, by anyone who was not on the job. Twenty-six of the 47 samples are measured in inches, so it is worth the two words. Both pairs say it, since a heading is read where the eye lands rather than from the left edge; the areas carry their own unit already.
...
- **Inches are typed in one box, at whatever fraction they were measured to.** `42 3/4`, `421/4` and `42.75` all mean the same size, and halves through sixty-fourths are all kept and written back exactly — a sixteenth is a sixteenth, not the nearest eighth, because rounding for the sake of a tidier column prices the row on a size the customer never gave. A size that is no fraction of a tape at all is written as the decimal it is. One box, no second box and no dropdown: the fraction is written on the shop's own drawings the same way it is typed here.
- **A box is never rewritten while it is being typed into.** On the way to `42 11/16` the text passes through `42 11/1`, which is a real size — forty-two and eleven over one, or 53 — and rewriting the box on that reading put `536` on the row. A box now takes a new value only from outside it: a reset, or a change of unit.
